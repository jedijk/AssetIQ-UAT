"""RIL dashboard payload builders — materialized read model (WS6)."""
...
async def build_ril_executive_payload(
    ...
    total_equipment = await db.equipment_nodes.count_documents(
        scoped(user, {"level": {"$in": equipment_levels}})
    )
...
async def build_ril_intelligence_payload(user: dict, owner_id: str) -> Dict[str, Any]:
...
async def build_ril_data_quality_payload(user: dict, owner_id: str) -> Dict[str, Any]:
    pipeline = [
        {"$match": scoped(user, {"owner_id": owner_id})},
        {"$group": {
            "_id": "$source",
    ...
    last_observation = await db.ril_observations.find_one(
        scoped(user, {"owner_id": owner_id}), sort=[("created_at", -1)]
    )
    last_reading = await db.ril_readings.find_one(
        scoped(user, {"owner_id": owner_id}), sort=[("received_at", -1)]
    )
    last_alert = await db.ril_alerts.find_one(
        scoped(user, {"owner_id": owner_id}), sort=[("received_at", -1)]
    )

    total_equipment = await db.equipment_nodes.count_documents(scoped(user, {"owner_id": owner_id}))
    equipment_with_observations = await db.ril_observations.distinct(
        "equipment_id",
        scoped(user, {"owner_id": owner_id, "equipment_id": {"$ne": None}}),
    )
