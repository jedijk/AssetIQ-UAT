...
async def update_action(
    ...
    if status_changed_to_completed and action.get("source_type") and action.get("source_id"):
        ...
        remaining_open = await _action_repo.count_open_for_source(source_type, source_id, user=current_user)

        if remaining_open == 0:
            total_actions = await _action_repo.count_for_source(source_type, source_id, user=current_user)
            source_name = None
            source_status = None
            ...
            if source_status not in ["closed", "completed"]:
                completion_notification = {
                    ...
                    "message": (
                        f"All {total_actions} action(s) for '{source_name}' are now complete! "
                        f"Consider closing this {source_type.replace('threat', 'observation')}."
                    ),
