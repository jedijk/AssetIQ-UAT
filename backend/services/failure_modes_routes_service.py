"""Failure modes library routes — service layer."""
...
async def get_failure_modes(
    ...
    skip: int = 0,
    limit: int = 500, *, current_user: dict
):
...
async def get_high_risk_modes(
    threshold: int = 150, *, current_user: dict
):
...
async def get_similar_failure_modes(
    ...
    limit: int = 20,
    require_shared_equipment_type: bool = False, *, current_user: dict
):
...
async def create_failure_mode(
    data: FailureModeCreate, *, current_user: dict
):
...
async def update_failure_mode(
    mode_id: str,
    data: FailureModeUpdate, *, current_user: dict
):
...
async def rollback_failure_mode(
    mode_id: str,
    data: RollbackRequest, *, current_user: dict
):
...
async def validate_failure_mode(
    mode_id: str,
    data: FailureModeValidation, *, current_user: dict
):
...
async def scan_similar_failure_modes(
    request: FindSimilarFailureModesScanRequest, *, current_user: dict
):
...
async def merge_duplicate_actions(
    request: MergeDuplicateActionsRequest, *, current_user: dict
):
...
async def merge_failure_modes(
    request: MergeFailureModesRequest, *, current_user: dict
):
