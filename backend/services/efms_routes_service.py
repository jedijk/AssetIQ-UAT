"""Equipment failure modes (EFM) routes — service layer."""
...
async def get_equipment_efms(
    ...
    category: Optional[str] = None, *, current_user: dict
):
...
async def get_high_risk_efms(
    ...
    min_rpn: int = 150, *, current_user: dict
):
...
async def update_efm(
    efm_id: str,
    data: EFMUpdate, current_user: dict
):
