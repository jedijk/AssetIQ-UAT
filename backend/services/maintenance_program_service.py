...
Only include tasks that would genuinely improve reliability and are not redundant with existing tasks."""
    ...
    @staticmethod
    def _existing_strategy_template_ids(program_tasks: List[Dict[str, Any]]) -> set:
        template_ids = set()
        for task in program_tasks:
    ...
    async def enrich_program_response_with_strategy_tasks(
        ...
        existing_template_ids = MaintenanceProgramService._existing_strategy_template_ids(
            program.get("tasks") or []
        )
        new_strategy_tasks = await MaintenanceProgramService.generate_tasks_from_strategy(
            equipment_type_id=equipment_type_id,
