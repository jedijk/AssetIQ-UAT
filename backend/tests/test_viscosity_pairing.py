...
async def test_viscosity_pairing():
    ...
    if not ext_tpl or not visc_tpl:
        pytest.skip("Extruder/Mooney form templates not seeded")
    ...
    try:
        ...
        await _mk_ext(test_date.replace(hour=14, minute=30))
        ...
        await _mk_ext(other_day.replace(hour=9, minute=0))
        vid4, vdoc4 = await _mk_visc(datetime(2026, 1, 7, 10, 0))
