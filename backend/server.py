...
# Initialize EFM Service (Equipment Failure Modes)
...
async def recalculate_threat_scores_for_asset(asset_name: str, user_id: str, new_criticality: dict = None, equipment_node_id: str = None):
    ...
    Formula: Risk Score = (Criticality × 0.75) + (Likelihood Score × 0.25)
    
    Finds threats by:
    1. asset name match
    2. linked_equipment_id match
    """
    # Build query to find threats - match by asset name OR linked_equipment_id
    ...
    if new_criticality:
        ...
        # Determine criticality level based on max impact
    ...
    for threat in threats:
        # Get FMEA/Likelihood score (from linked failure mode or stored score)
        ...
        if failure_mode_name and failure_mode_name != "Unknown":
            ...
            for fm in FAILURE_MODES_LIBRARY:
                if fm["failure_mode"].lower() == failure_mode_name.lower():
                    # Calculate FMEA score: (S × O × D) / 10, capped at 100
                    fmea_score = min(100, int(fm["rpn"] / 10))
                    break
        
        # NEW METHODOLOGY: Risk Score = (Criticality × 0.75) + (Likelihood Score × 0.25)
        ...
        # Update threat with recalculated scores
        update_data = {
        ...
        # Add criticality data if available
        if criticality_data:
...
async def recalculate_threat_scores_for_failure_mode(failure_mode_name: str, new_severity: int, new_occurrence: int, new_detectability: int):
    ...
    Formula: Risk Score = (Criticality × 0.75) + (Likelihood Score × 0.25)
    """
    # Find all threats with this failure mode (case-insensitive)
    threats = await db.threats.find(
    ...
    # Calculate new FMEA/Likelihood score from RPN: (S × O × D) / 10
    ...
    for threat in threats:
        ...
        # CORRECT FORMULA: Risk Score = (Criticality × 0.75) + (Likelihood Score × 0.25)
...
@api_router.get("/equipment/{equipment_id}/efms")
...
@api_router.get("/equipment/{equipment_id}/efms/summary")
...
@api_router.get("/equipment/{equipment_id}/risk")
...
@api_router.post("/equipment/{equipment_id}/efms/generate")
...
@api_router.post("/efms/{efm_id}/reset")
...
@api_router.get("/task-templates/{template_id}")
...
@api_router.patch("/task-templates/{template_id}")
...
@api_router.get("/task-plans/{plan_id}")
...
@api_router.patch("/task-plans/{plan_id}")
...
@api_router.delete("/task-plans/{plan_id}")
...
@api_router.get("/task-instances/calendar")
...
@api_router.get("/task-instances/{instance_id}")
...
@api_router.post("/task-instances")
...
@api_router.patch("/task-instances/{instance_id}")
...
@api_router.post("/task-instances/{instance_id}/start")
...
@api_router.post("/task-instances/{instance_id}/complete")
...
@api_router.post("/tasks/generate-all")
...
@api_router.post("/tasks/mark-overdue")
...
@api_router.get("/form-templates/{template_id}")
...
@api_router.get("/form-templates/{template_id}/versions")
...
@api_router.post("/form-templates")
...
@api_router.patch("/form-templates/{template_id}")
...
@api_router.post("/form-templates/{template_id}/fields")
...
@api_router.patch("/form-templates/{template_id}/fields/{field_id}")
...
@api_router.delete("/form-templates/{template_id}/fields/{field_id}")
...
@api_router.post("/form-templates/{template_id}/fields/reorder")
...
@api_router.get("/form-templates/{template_id}/analytics")
...
@api_router.get("/decision-engine/dashboard")
...
@api_router.get("/decision-engine/rules")
...
@api_router.patch("/decision-engine/rules/{rule_id}")
...
@api_router.post("/decision-engine/evaluate")
...
@api_router.get("/decision-engine/suggestions")
...
@api_router.post("/decision-engine/suggestions/{suggestion_id}/approve")
...
@api_router.post("/decision-engine/suggestions/{suggestion_id}/reject")
...
@api_router.post("/decision-engine/suggestions/{suggestion_id}/execute")
...
@api_router.get("/analytics/risk-overview")
...
@api_router.get("/analytics/failure-mode-pareto")
...
@api_router.get("/analytics/task-compliance")
...
@api_router.get("/analytics/task-workload")
...
@api_router.get("/analytics/detection-effectiveness")
...
@api_router.get("/analytics/equipment-risk-ranking")
...
@api_router.get("/analytics/under-controlled-risks")
...
@api_router.get("/analytics/over-maintained-assets")
...
@api_router.get("/analytics/form-threshold-summary")
...
@api_router.get("/rbac/users/{user_id}")
...
@api_router.patch("/rbac/users/{user_id}/role")
...
@api_router.patch("/rbac/users/{user_id}/status")
...
@api_router.patch("/rbac/users/{user_id}/profile")
...
@api_router.get("/rbac/role-distribution")
