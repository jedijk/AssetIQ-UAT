#!/usr/bin/env python3
"""Wave 4 tenant isolation and strict-mode readiness report."""
...
from services.tenant_schema import WAVE4_COLLECTIONS  # noqa: E402


async def main() -> int:
    print("=== Wave 4 Tenant Isolation Report ===\n")
    ...
    ok, rows = await wave_coverage(db, WAVE4_COLLECTIONS)
    print(f"Wave 4 backfill: {'OK' if ok else 'GAPS'}")
    ...
    audit = [r for r in await build_isolation_report(db) if r.get("wave") == "wave4"]
    if audit:
        print("\nWave 4 collection audit:")
        for row in audit:
            print(
