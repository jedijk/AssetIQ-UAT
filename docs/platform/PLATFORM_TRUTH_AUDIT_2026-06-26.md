...
| **SpareIQ** | Spare parts, requirements | `spare_parts` | `spare_parts`, `spare_categories` | `spare_parts_service`, graph sync | **P** | Domain registry; spare tests exist | NOT VERIFIED: customer usage depth |
...
| **Visual Management Boards** | Gemba / TV boards | `visual_boards` | `visual_boards`, tokens, pairings | `visual_board_service` | **I** | Domain registry; VMB tests in frontend | Kiosk/TV edge cases |
...
| Program → Planned work | Deterministic | `scheduled_tasks` generation | Schedule drift script exists |
| Planned work → Task instance | Deterministic + job | `backfill_scheduled_task_instances.py`; bridge gate | Was 248 gaps; UAT now 0 |
...
| Scheduler tenant scope | **P** | Maintenance scheduling uses tenant helpers |
...
*This document is the product + platform truth snapshot as of 2026-06-26. Re-verify UAT gates after material changes to scheduler, graph, or tenant scope.*
