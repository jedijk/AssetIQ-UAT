 ...
 * Filter controls for Actions page
 */
    ...
    <div className="flex items-center gap-3 flex-wrap">
      ...
      <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1">
        {["all", "open", "in_progress", "completed"].map((status) => (
          ...
          >
            {t?.(`actions.${status}`) || status.replace("_", " ")}
          </Button>
        ))}
      </div>
        ...
        <PopoverContent className="w-64" align="end">
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">{t?.("common.priority") || "Priority"}</label>
            ...
            {users.length > 0 && (
              <div className="space-y-2">
                <label className="text-sm font-medium">{t?.("common.assignee") || "Assignee"}</label>
