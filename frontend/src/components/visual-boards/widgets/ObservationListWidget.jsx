...
const ObservationListWidget = ({ widget, data }) => {
  ...
  return (
    <div className="h-full rounded-xl border border-slate-700/50 bg-slate-900/80 p-4 flex flex-col overflow-hidden">
      <div className="text-sm font-semibold text-white mb-3">{widget?.title || "Observations"}</div>
      <div className="flex-1 overflow-auto space-y-2">
        {items.length === 0 ? (
          <div className="text-sm text-slate-500">No active observations</div>
        ) : (
          items.map((item) => (
            <div
              key={item.id || item.observation_id}
              className="rounded-lg bg-slate-800/80 px-3 py-2 text-sm"
            >
