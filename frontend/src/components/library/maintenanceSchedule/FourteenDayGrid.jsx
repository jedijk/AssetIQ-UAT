...
export function FourteenDayGrid({ buckets, totalTasks }) {
  const { t } = useLanguage();
  const taskWord =
    totalTasks === 1 ? t("maintenance.taskSingular") : t("maintenance.tasks");

  return (
    <div className="space-y-2">
      <div className="text-xs text-slate-500" data-testid="planner-summary-14">
        {totalTasks} {taskWord} {t("maintenance.scheduledIn14Days")}
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
        {buckets.map((b) => {
          ...
          const bucketTaskWord =
            b.tasks.length === 1 ? t("maintenance.taskSingular") : t("maintenance.tasks");
          return (
