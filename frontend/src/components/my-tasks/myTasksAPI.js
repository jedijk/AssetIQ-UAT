...
export const myTasksAPI = {
  ...
  uploadAttachment: async (file) => {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/tasks/upload-attachment`,
      getAuthFetchInit({ method: "POST", body: formData })
    );
  ...
  executeAdhocPlan: async (planId) => {
    const response = await fetch(
      `${API_BASE_URL}/api/adhoc-plans/${planId}/execute`,
  ...
  deleteTask: async (taskId) => {
    const response = await fetch(
      `${API_BASE_URL}/api/my-tasks/${taskId}`,
